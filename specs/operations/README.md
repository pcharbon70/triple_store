...
- `Snapshot` is globally supervised, while statistics helpers and scheduled
  backup remain dynamic or caller-managed. A scheduled-backup process monitors
  its store lifecycle and terminates automatically after store closure.
...
| `AC-OPS-03` | Full-store backup, graph backup, restore, and scheduled backup remain documented as current runtime features, including scheduler termination with its store lifecycle. | `REQ-OBS-*`, `REQ-STO-*` | `SCN-012` |
