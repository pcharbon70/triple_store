...
graph TD
  ...
  F["Backup / Restore"] --> G["ScheduledBackup"]
...
- Backup and scheduled-backup support are current production-facing features, including incremental backup and rotation behavior.
...
| `AC-OPS-03` | Backup, restore, incremental backup, and scheduled backup remain documented as current runtime features. | `test/triple_store/backup_test.exs`, `test/triple_store/scheduled_backup_test.exs` |
...
| `AC-OPS-05` | The specs capture the current split between deprecated `Statistics.Cache` integration and the newer `Statistics.Server` implementation. | `test/triple_store/statistics/cache_test.exs`, `test/triple_store/statistics/server_test.exs` |
