defmodule TripleStore.Backend.RocksDB.IteratorOperationsTest do
  ...
  describe "2.1.1 Basic Iterator Creation" do
    ...
    test "2.1.1.2 supports fill_cache option", %{db: db} do
      {:ok, iter} = ErlangAdapter.prefix_iterator(db, :spo, <<>>, fill_cache: false)
      assert is_pid(iter)
      ErlangAdapter.iterator_close(iter)
    end
  ...
  describe "2.1.2 Prefix Iterator Migration" do
    test "2.1.2.1 prefix_iterator with prefix bounds checking", %{db: db} do
      ...
      {:ok, key, _value} = ErlangAdapter.iterator_move(iter, <<1::64-big>>)
    ...
    test "2.1.2.2 respects total_order_seek option", %{db: db} do
      ...
      {:ok, iter} = ErlangAdapter.prefix_iterator(db, :spo, <<>>, total_order_seek: true)

      # Seek to middle of data
      assert {:ok, _key, _value} = ErlangAdapter.iterator_move(iter, <<2::64-big>>)
    ...
    test "2.1.2.5 maintains prefix boundary checking", %{db: db} do
      ...
      ErlangAdapter.iterator_seek(iter, <<2::64-big>>)
  ...
  describe "2.1.3 Seek Operations Migration" do
    test "2.1.3.1 implements iterator_seek", %{db: db} do
      ...
      assert :ok = ErlangAdapter.iterator_seek(iter, <<1::64-big, 2::64-big, 1::64-big>>)

      # Next should return the key at or after seek position
      assert {:ok, <<1::64-big, 2::64-big, _::binary>>, _} = ErlangAdapter.iterator_next(iter)
    ...
    test "2.1.3.2 handles seek with non-existent keys", %{db: db} do
      ...
      assert :ok = ErlangAdapter.iterator_seek(iter, <<1::64-big, 3::64-big, 0::64-big>>)

      # Should position at next available key
      assert {:ok, <<2::64-big, _::binary>>, _} = ErlangAdapter.iterator_next(iter)
    ...
    test "2.1.3.3 handles seek past end of data", %{db: db} do
      ...
      # Seek past all data
      assert :ok = ErlangAdapter.iterator_seek(iter, <<255::64-big>>)
    ...
    test "2.1.3.4 handles seek to first key", %{db: db} do
      ...
      # Seek to first key
      assert :ok = ErlangAdapter.iterator_seek(iter, <<0::64-big>>)

      # Should get first entry
      assert {:ok, <<1::64-big, _::binary>>, _} = ErlangAdapter.iterator_next(iter)
    ...
    test "2.1.3.5 validates Leapfrog operations", %{db: db} do
      ...
      # Seek to subject=1
      assert :ok = ErlangAdapter.iterator_seek(iter, <<1::64-big>>)
      assert {:ok, <<1::64-big, _::binary>>, _} = ErlangAdapter.iterator_next(iter)

      # Seek to subject=2
      assert :ok = ErlangAdapter.iterator_seek(iter, <<2::64-big>>)
      assert {:ok, <<2::64-big, _::binary>>, _} = ErlangAdapter.iterator_next(iter)

      # Seek to subject=3
      assert :ok = ErlangAdapter.iterator_seek(iter, <<3::64-big>>)
      assert {:ok, <<3::64-big, _::binary>>, _} = ErlangAdapter.iterator_next(iter)

      # Seek past end
      assert :ok = ErlangAdapter.iterator_seek(iter, <<4::64-big>>)
      assert :iterator_end = ErlangAdapter.iterator_next(iter)
  ...
  describe "2.1.4 Iterator Collect Operation" do
    ...
    test "2.1.4.4 handles exhausted iterator", %{db: db} do
      {:ok, iter} = ErlangAdapter.prefix_iterator(db, :spo, <<255::64-big>>)
  ...
  describe "2.1.5 Additional Tests" do
    test "2.1.5.1 iterator_next returns entries sequentially", %{db: db} do
      ...
      # Get first entry
      {:ok, key1, _} = ErlangAdapter.iterator_next(iter)

      # Get second entry
      {:ok, key2, _} = ErlangAdapter.iterator_next(iter)
    ...
    test "2.1.5.2 iterator_move with binary seek key", %{db: db} do
      ...
      {:ok, _key, _value} = ErlangAdapter.iterator_move(iter, <<2::64-big>>)

      # Verify we're at expected position
      {:ok, <<2::64-big, _::binary>>, _} = ErlangAdapter.iterator_next(iter)
    ...
    test "2.1.5.3 multiple iterators can coexist", %{db: db} do
      ...
      {:ok, entries1} = ErlangAdapter.iterator_collect(iter1)
      ...
      {:ok, entries2} = ErlangAdapter.iterator_collect(iter2)
    ...
    test "2.1.5.4 iterator handles empty column family", %{db: db} do
      ...
      {:ok, iter} = ErlangAdapter.prefix_iterator(db, :id2str, <<>>)
    ...
    test "2.1.5.5 iterator with single entry", %{db: db} do
      ...
      :ok = ErlangAdapter.put(db, :str2id, <<42::64-big>>, <<"test">>)

      {:ok, iter} = ErlangAdapter.prefix_iterator(db, :str2id, <<>>)
    ...
    test "2.1.5.6 iterator maintains position across operations", %{db: db} do
      ...
      # Move to first
      {:ok, k1, _} = ErlangAdapter.iterator_move(iter, :first)

      # Move to next
      {:ok, k2, _} = ErlangAdapter.iterator_move(iter, :next)

      # Move back to prev
      {:ok, k1_again, _} = ErlangAdapter.iterator_move(iter, :prev)
